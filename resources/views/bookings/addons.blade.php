...
<div class="container py-4" style="max-width:900px;">
    ...
    @if($hotels->isEmpty())
        <div class="alert alert-light border mb-4">No hotels available at this destination yet.</div>
    @else
    <div class="row g-3 mb-5">
        ...
        <div class="col-md-6">
            <div class="card border-0 shadow-sm h-100">
                        ...
                        <div class="card-body d-flex flex-column h-100 py-2 px-3">
                            ...
                            <div class="mt-auto">
                                <button class="btn btn-primary btn-sm w-100"
                                        ...
                                        data-bs-target="#hotelRooms{{ $hotel->id }}">
                                    Choose a room
                                </button>
                            </div>
                        </div>
                ...
                {{-- Room list collapse --}}
                <div class="collapse" id="hotelRooms{{ $hotel->id }}">
                    <div class="card-body pt-0 border-top">
                        @foreach($hotel->rooms->where('available_rooms', '>', 0) as $room)
                        <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                            <div>
                                <div class="fw-semibold small">{{ ucfirst($room->type) }} · {{ $room->capacity }} guests</div>
                            ...
                            <div class="text-end">
                                <div class="small fw-bold text-primary">{{ number_format($room->price_per_night, 2) }} DA/night</div>
                                ...
                                <div class="modal-content">
                                    <form action="{{ route('hotels.book') }}" method="POST">
                                        ...
                                        <div class="modal-header">
                                            <h5 class="modal-title">{{ ucfirst($room->type) }} Room — {{ $hotel->name }}</h5>
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="submit" class="btn btn-primary">Add to my trip</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
            ...
            <div class="card border-0 shadow-sm h-100">
                @if($car->image)
                    <img src="{{ asset('storage/' . $car->image) }}"
                         class="card-img-top" style="height:140px;object-fit:cover;" alt="{{ $car->brand }}">
                @else
                ...
                <div class="card-body">
                    ...
                    <div class="text-muted small mb-2">{{ ucfirst($car->type) }} · {{ $car->seats }} seats</div>
                    <div class="small d-flex gap-2 flex-wrap mb-2">
                        <span>⚙️ {{ ucfirst($car->transmission) }}</span>
                        <span>⛽ {{ ucfirst($car->fuel) }}</span>
                        @if($car->with_ac)<span>❄️ AC</span>@endif
                    </div>
                    <div class="fw-bold text-primary">{{ number_format($car->price_per_day, 2) }} DA/day</div>
                </div>
