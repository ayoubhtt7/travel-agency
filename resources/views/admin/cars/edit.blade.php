...
<div class="container py-4" style="max-width:700px;">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <h2 class="mb-0">Edit — {{ $car->brand }} {{ $car->model }}</h2>
    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="type" class="form-select" required>
                            @foreach(['economy','compact','suv','luxury','van','convertible'] as $t)
                                <option value="{{ $t }}" {{ old('type', $car->type) === $t ? 'selected' : '' }}>{{ ucfirst($t) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="fuel" class="form-select">
                            @foreach(['petrol','diesel','electric','hybrid'] as $f)
                                <option value="{{ $f }}" {{ old('fuel', $car->fuel) === $f ? 'selected' : '' }}>{{ ucfirst($f) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <select name="destination_id" class="form-select">
                        ...
                        @foreach($destinations as $dest)
                            <option value="{{ $dest->id }}"
                                    {{ old('destination_id', $car->destination_id) == $dest->id ? 'selected' : '' }}>
                ...
                <div class="mb-3 d-flex gap-4">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="with_ac" value="1"
                               id="with_ac" {{ old('with_ac', $car->with_ac) ? 'checked' : '' }}>
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="unlimited_mileage" value="1"
                               id="unlimited_mileage" {{ old('unlimited_mileage', $car->unlimited_mileage) ? 'checked' : '' }}>
                    ...
                    @if($car->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $car->image) }}" class="rounded"
                                 style="height:100px;object-fit:cover;" alt="Current image">
